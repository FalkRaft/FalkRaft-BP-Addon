...
import { tick } from "../tick-event.js";

export function antiNuker() {
  ...
  world.afterEvents.playerBreakBlock.subscribe(
    ({ block, brokenBlockPermutation, dimension, player }) => {
      ...
      if ((old.time ?? Date.now()) < Date.now() - 50)
        return blockLog.set(player.name, {
          location: block.location,
          permutation: brokenBlockPermutation,
        });
      if (blockLog.has(player.name) && log.get(player.name).amount === 0) {
        dimension
          .getBlock(blockLog.get(player.name).location)
          .setPermutation(blockLog.get(player.name).permutation);
        setTickTimeout(() => {
          dimension
            .getEntitiesAtBlockLocation(
              blockLog.get(player.name)?.location ?? block.location
            )
  ...
  world.afterEvents.playerLeave.subscribe(
    (data) => log.delete(data.playerName) && blockLog.delete(data.playerName)
  );
